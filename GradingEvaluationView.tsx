...
export const GradingEvaluationView: React.FC<GradingEvaluationViewProps> = ({ classrooms }) => {
  ...
  const handlePlayTTS = (text: string) => {
    ...
    setAudioStatusMsg('🔊 Đang phát giọng đọc Tiếng Anh chuẩn quốc tế (US English)...');
  };
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 text-white rounded-2xl p-5 sm:p-6 border border-cyan-500/30 shadow-xl relative overflow-hidden">
        ...
        {/* Filters Bar */}
        <div className="mt-5 pt-4 border-t border-cyan-500/20 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-[11px] font-bold text-slate-400 block mb-1">1. Chọn Lớp học</label>
            <select
              ...
              className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-xs font-bold text-white focus:border-cyan-400 focus:outline-none cursor-pointer"
            >
              {classrooms.map((cls) => (
          ...
          <div>
            <label className="text-[11px] font-bold text-slate-400 block mb-1">2. Chọn Học sinh chấm bài</label>
            <select
          ...
          <div>
            <label className="text-[11px] font-bold text-slate-400 block mb-1">3. Môn học đánh giá (Đầy đủ tất cả các môn)</label>
            <select
              value={selectedSubject}
              onChange={(e) => setSelectedSubject(e.target.value)}
              className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-xl text-xs font-bold text-amber-300 focus:border-cyan-400 focus:outline-none cursor-pointer"
            >
